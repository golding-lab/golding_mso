...
class SynapseTerminal:
    ...
    def __init__(self, segment: Segment, **kwargs) -> None:
        ...
        self.fiber = kwargs.pop("fiber", None)  # Reference to SynapseFiber if part of one
...
class SynapseFiber:
    ...
    def __init__(self, synapse_terminals: list[SynapseTerminal], **kwargs) -> None:
        ...
        self.shared_firing_times = kwargs.pop("firing_times", [0])
        self.fake_cell = h.VecStim()
        self.set_firing_times(self.shared_firing_times)
        self.rzero = kwargs.pop("rzero", None)  # Default rzero value if not provided
        
        for syn_terminal in self.synapse_terminals:
            syn_terminal.fake_cell = self.fake_cell
            syn_terminal.fiber = self  # Set reference to this fiber in each terminal
    ...
    def set_firing_times(self, firing_times: list[float] = None) -> None:
        ...
        for syn_terminal in self.synapse_terminals:
            syn_terminal.firing_times = firing_times
            firing_times = np.array(self.shared_firing_times)
            syn_terminal.create_netcon()  # Recreate NetCon to connect to the new fake_cell
        self.fake_cell.play(h.Vector(firing_times.flatten()), 1)
        self.shared_firing_times = firing_times
...
class StochasticSynapseTerminal(SynapseTerminal):
    ...
    def _compute_pool_depletion(self, firing_times=None):
        ...
        releases = np.array(list(self.vesicle_pool.serial_release(firing_times)))
        self.cond_values = releases * self._gmax / self.vesicle_pool.rzero
    ...
    def set_firing_times(self, firing_times: list[float] = None) -> None:
        ...
        if self.fiber is None:
            self.fake_cell.play(self._tvec)  # Use VecStim to trigger NetCon at firing times
