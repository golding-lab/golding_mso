...
class Cell:
    ...
    def __init__(self, cell_file: str, config_path: str = None, **kwargs) -> None:
        """
        ...
        """
        self._load_default_values(config_path=config_path, **kwargs)
    ...
    def _create_axon_sections(self, num_nodes=5, tais_max_diam = 1.64, cais_diam = 0.66) -> None:
        self.axonal = []
        ...
        for segnum, seg in enumerate(self.tais):
            ...
            seg.diam = tais_max_diam - (((segnum + 1) / self.tais.nseg) * (tais_max_diam - cais_diam))
...
#TODO slow
    def _get_threshold(
        ...
        while (
            ...
            if (
                ...
                if max_depol_without_spike < max(ax_voltage_vector) - ax_voltage_vector[0]:
                    max_depol_without_spike = max(soma_voltage_vector) - soma_voltage_vector[0]
            (
            ...
            logger.debug(f"Spike pair: {current_range[0]} nA: {spike_pair[0]}, {current_range[1]} nA: {spike_pair[1]}")
    ...
    def _load_default_values(self, config_path: str = None, **kwargs) -> None:
        """
    ...
    def _set_channel_cond(
        ...
        if isinstance(input_cond, bytes):    
            if dill_import:
                ...
                if callable(input_cond): input_cond = input_cond(self)
            else: 
                raise ValueError(
    ...
    def surface_area(
        ...
        if getattr(self, "_surface_area", None) is None:
            ...
            area = 0
            for sec in [sec for sec in self.allsec_nofilopodia if sec not in getattr(self,'axonal', [])]:
                for seg in sec:
                    area += seg.area()
