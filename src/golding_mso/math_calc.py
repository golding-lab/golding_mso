...
@overload
def perpendicular_distance_point_to_line(line: tuple[float, float], point: tuple[float, float],) -> float:
    x0, y0 = point
    m,b = line
    x1, y1 = (0, b)
    x2, y2 = (1, m+b)
...
@overload
def perpendicular_distance_point_to_line(line: tuple[tuple[float, float], tuple[float, float]], point: tuple[float, float], ) -> float:
    x0, y0 = point
    x1, y1 = line[0]
    x2, y2 = line[1]
...
def perpendicular_distance_point_to_line(line, point) -> float:
    ...
    """
    if isinstance(line, tuple) and len(line) == 2 and all(isinstance(pt, tuple) and len(pt) == 2 for pt in line):
        x0, y0 = point
        x1, y1 = line[0]
        x2, y2 = line[1]
    else:
...
def perpendicular_distance_point_to_line_3d(point: tuple[float, float, float], 
                                           line_point1: tuple[float, float, float], 
                                           line_point2: tuple[float, float, float]) -> float:
    """
